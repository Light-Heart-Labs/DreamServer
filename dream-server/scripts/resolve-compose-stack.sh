...
# Proxy-exclusive overlay — strips host port bindings from every service
# fronted by dream-proxy when DREAM_PROXY_EXCLUSIVE=true. Generated by
# scripts/resolve-proxy-config.sh, which inspects manifest `proxy:` blocks
# and writes a `ports: []` override for each routed service. We append it
# late so it wins the merge; the resolver removes the file when the flag
# is off so a stale overlay can't sneak back in.
proxy_exclusive_env = os.environ.get("DREAM_PROXY_EXCLUSIVE", "").strip().lower() in {"1", "true", "yes", "on"}
proxy_exclusive_path = script_dir / "docker-compose.proxy-exclusive.yml"
if proxy_exclusive_env and proxy_exclusive_path.exists():
    resolved.append("docker-compose.proxy-exclusive.yml")

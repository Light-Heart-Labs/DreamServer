...
def read_env() -> dict[str, str]:
    ...
    double-quotes are stripped from values so callers always get the raw string.
    """
    env: dict[str, str] = {}
...
def write_env_key(key: str, value: str) -> None:
    ...
    so line order and comments are preserved.  No shell evaluation is performed.
    """
    if not _ENV_PATH.exists():
...
def validate_against_schema(
    ...
    Supported constraints: enum, minimum, maximum, type (number coercion only).
    """

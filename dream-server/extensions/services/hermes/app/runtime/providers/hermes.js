...
const { ensureWorkingLocalBaseUrl, ensureWorkingLocalModel, requestBaseUrlForRuntime } = require("./local");
const os = require("os");
...
function looksLikeLocalBaseUrl(value = "") {
  return /^(https?:\/\/)?(localhost|host\.docker\.internal|127(?:\.\d{1,3}){3}|0\.0\.0\.0|\[::1\])(?::\d+)?(?:\/|$)/i.test(String(value || "").trim());
}
...
function looksLikeDreamServerLocalServiceBaseUrl(value = "") {
  try {
    const parsed = new URL(String(value || "").trim());
    return new Set(["litellm", "llama-server", "ollama", "localai", "vllm"]).has(parsed.hostname.toLowerCase());
  } catch {
    return false;
  }
}
...
async function resolveLocalEndpointForHermes(settings = {}) {
  const provider = normalizeHermesProvider(settings.hermesProvider || "custom");
  if (!PROVIDERS_ALLOW_LOCAL_BASE_URL.has(provider)) {
    return settings;
  }

  const baseUrl = String(settings.localBaseUrl || "").trim();
  if (
    baseUrl &&
    !looksLikeLocalBaseUrl(baseUrl) &&
    !looksLikeDefaultLocalBaseUrl(baseUrl) &&
    !looksLikeDreamServerLocalServiceBaseUrl(baseUrl)
  ) {
    return settings;
  }

  await ensureWorkingLocalBaseUrl(settings, { timeoutMs: 2500 }).catch(() => {});
  await ensureWorkingLocalModel(settings).catch(() => {});
  return settings;
}

function buildHermesRequestOverrides(settings = {}, routing = resolveHermesRoutingSettings(settings)) {
  if (!PROVIDERS_WITH_CHAT_TEMPLATE_THINKING.has(routing.provider)) {
    return null;
  }
  const thinkingEnabled = Boolean(settings?.localThinkingEnabled);
  const localEndpointRoute = PROVIDERS_ALLOW_LOCAL_BASE_URL.has(routing.provider) &&
    (
      looksLikeLocalBaseUrl(routing.baseUrl) ||
      looksLikeDefaultLocalBaseUrl(routing.baseUrl) ||
      looksLikeDreamServerLocalServiceBaseUrl(routing.baseUrl)
    );
  if (!thinkingEnabled && !localEndpointRoute) {
    return null;
  }
  return {
    extra_body: {
...
async function sendHermesTurn({
  ...
  await resolveLocalEndpointForHermes(settings);
  ...
  const buildRequest = (overrides = {}) => ({
    inputText,
    workspaceRoot,
    baseUrl: requestBaseUrlForRuntime(hermesRouting.baseUrl),
...
module.exports = {
  ...
  _test: {
    ...
    looksLikeDreamServerLocalServiceBaseUrl,
    resolveLocalEndpointForHermes,

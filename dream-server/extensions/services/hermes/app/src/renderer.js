const DREAM_DESKTOP_NAME = "Dream Server DESKTOP";
const DEFAULT_AMBIENT_IMAGE = "./assets/dream-desktop-background.jpg";
const DEFAULT_AMBIENT_IMAGE_LABEL = "dream-desktop-background.jpg";
...
const elements = {
  ...
  dreamPetOverlay: document.getElementById("dreamPetOverlay"),
  dreamPetHandle: document.getElementById("dreamPetHandle"),
  dreamPetSprite: document.getElementById("dreamPetSprite"),
  dreamPetBubble: document.getElementById("dreamPetBubble"),
  dreamPetStatusText: document.getElementById("dreamPetStatusText"),
...
const DREAM_PET_FRAME = { width: 192, height: 208, columns: 8 };
const DREAM_PET_ROWS = {
...
const DREAM_PET_LINES = {
  idle: [
    "Dreamserver online.",
    "Estou aqui, Gabriel.",
    "KIMI pronto para acelerar."
  ],
  waiting: [
    "Fico de guarda enquanto voce decide o proximo passo.",
    "Tudo calmo por aqui.",
    "Pode me arrastar para qualquer canto."
  ],
  running: [
    "Estou acompanhando a execucao.",
    "Processando com foco.",
    "Segurando o ritmo do Dream Server."
  ],
  review: [
    "Tem coisa pronta para revisar.",
    "Vou ficar de olho nos detalhes.",
    "Modo revisao ligado."
  ],
  failed: [
    "Algo travou. Vamos diagnosticar.",
    "Falha detectada, mas nada fica sem pista.",
    "Respira. Eu marco o ponto que precisa de atencao."
  ],
  waving: [
    "Oi, eu sou a Dreamserver.",
    "Pet ativado.",
    "Voltei para a tela."
  ],
  jumping: [
    "Bora.",
    "Energia alta por aqui.",
    "Pronta para o proximo comando."
  ]
};
...
const HERMES_PROVIDER_LABELS = {
  auto: "Auto (config Dream Server DESKTOP)",
  custom: "Custom / local",
...
function currentHermesRoute(settings = state.app?.settings || {}) {
  ...
  const model = effectiveModelForProvider(provider, settings.localModel || "");
...
function executionProviderForSettings(settings = state.app?.settings || {}) {
  return routeUsesLocalEndpoint(settings) ? "local" : "cloud";
}
...
function aperantProviderHealth(provider = selectedAperantProvider()) {
  ...
  if (normalized === "cloud") {
    const route = currentHermesRoute(settings);
    const hasCloudRoute = !routeUsesLocalEndpoint(settings) && (route.provider === "auto" || Boolean(route.baseUrl));
    return hasCloudRoute
      ? { ok: true, label: `${route.label} ok`, detail: route.baseUrl || "Provider cloud via config do Dream Server DESKTOP." }
      : { ok: false, label: "Cloud nao configurado", detail: "Escolha um provider cloud em Settings > Dream Server DESKTOP." };
  }
  const route = currentHermesRoute(settings);
  ...
  if (settings.localLlamaEnabled) {
    ...
    if (["running", "starting"].includes(status) || settings.localLlamaAutoStart) {
      return {
        ok: true,
        label: status === "running" ? "Dream Server DESKTOP ok" : "Dream Server DESKTOP autostart",
  ...
  return {
    ...
    label: route.provider === "auto" ? "Dream Server DESKTOP config" : "Dream Server DESKTOP routing",
    detail: route.baseUrl || "Usando config.yaml/env do Dream Server DESKTOP."
  };
}
...
function showKanbanAccessRequired() {
  showToast("Ative acesso total em Settings para o Dream Server DESKTOP iniciar agentes.");
  openSettingsModal();
}
...
function homeTerminalLines() {
  ...
  const route = currentHermesRoute(settings);
  const localRoute = routeUsesLocalEndpoint(settings);
  const provider = String(route.label || route.provider || DREAM_DESKTOP_NAME).toUpperCase();
...
function renderHomeDashboard() {
  ...
  const localLlama = state.app?.localLlamaState || {};
  const managedLocalBaseUrl = settings.localLlamaEnabled ? String(localLlama.baseUrl || "").trim() : "";
  const tokenTelemetry = currentTokenTelemetry();
  ...
  setText(elements.opsLlamaStatus, localRoute
    ? String(tokenTelemetry.serverStatus || (settings.localLlamaEnabled ? localLlama.status : "") || "idle").toUpperCase()
    : String(tokenTelemetry.serverStatus || "cloud").toUpperCase());
  setText(elements.opsLlamaBase, localRoute
    ? managedLocalBaseUrl || route.baseUrl || "--"
...
async function pollHomeDashboard() {
  ...
  try {
    const metrics = await window.dreamDesktop.getSystemDashboard?.();
    if (metrics) {
...
function fileUrl(filePath) {
  ...
  if (/^(https?:|blob:|data:)/i.test(raw) || raw.startsWith("/uploads/")) {
    return raw;
  }
...
function applyAmbientBackground(filePath = "") {
  ...
  if (elements.backgroundMediaLabel) {
    elements.backgroundMediaLabel.textContent = hasCustom
      ? rawPath
      : `Padrao: ${DEFAULT_AMBIENT_IMAGE_LABEL}`;
  ...
  if (elements.ambientImage) {
    elements.ambientImage.src = useImage ? fileUrlFromPath(rawPath) : DEFAULT_AMBIENT_IMAGE;
...
async function ensureMobilePreviewService(forceRestart = false) {
  ...
  state.mobilePreview.promise = window.dreamDesktop.ensureMobilePreviewService({ forceRestart })
    .then((service) => {
...
function slashCatalogItems() {
  ...
  const localCommands = [
    ...
    { name: "/provider", title: "Provider Dream Server DESKTOP", description: "Troca provider Dream Server DESKTOP: kimi, nvidia, openai, claude, custom...", source: "dream" },
    { name: "/model", title: "Modelo Dream Server DESKTOP", description: "Troca o modelo ativo do Dream Server DESKTOP", source: "dream" },
...
function renderSlashCommandMenu() {
  ...
  const html = state.slashMenu.items.map((item, index) => `
    <button type="button" class="slash-command-item ${index === state.slashMenu.index ? "is-active" : ""}" data-slash-index="${index}">
      ...
      <span class="slash-command-source">${escapeHtml(item.source || item.category || DREAM_DESKTOP_NAME)}</span>
    </button>
  `).join("");
  menu.innerHTML = html;
  menu.hidden = !html;
...
function parseActionPayload(content) {
  ...
  const pattern = /```dream-server-action\s*([\s\S]*?)```/gi;
  const actions = [];
...
function attachmentMarkup(attachments = []) {
  const cards = attachments.map((attachment) => {
    const source = attachment.url || (attachment.path ? fileUrl(attachment.path) : "");
    const imageLike =
...
function visibleThinkingSummary(item = {}) {
  ...
  if (/^(thinking|activity|step|Hermes pensando|Dream Server DESKTOP pensando)$/i.test(summary)) {
    return "";
  }
...
function thinkingActivityMarkup() {
  ...
  return `
    <details class="thinking-details">
      <summary>Ver raciocinio do Dream Server DESKTOP</summary>
...
function syncPlatformSettingsHints(settings = {}) {
  ...
  if (elements.hermesDesktopIntegrationHint) {
    elements.hermesDesktopIntegrationHint.textContent =
      "Mantem as ferramentas Dream Desktop ativas para Workbench, navegador interno e caminhos do host. Recomendada em desktop; desligue apenas para modo puro sem ponte Dream.";
  }
  if (elements.localLlamaRuntimeDescription) {
    elements.localLlamaRuntimeDescription.textContent = managedAvailable
      ...
      : `Runtime llama.cpp gerenciado nao encontrado para ${label}. Use Dream Server DESKTOP routing com provider externo ou empacote bin/llama deste OS.`;
  }
  ...
  if (elements.localLlamaManagedHint) {
    elements.localLlamaManagedHint.textContent = managedAvailable
      ...
      : "Esta instalacao nao tem llama-server empacotado para este host. Dream Server DESKTOP ainda pode usar OpenRouter, APIs diretas, Ollama, vLLM, LM Studio ou outro endpoint.";
  }
...
function syncProviderSpecificSettings(settings = state.app?.settings || {}, options = {}) {
  ...
  if (elements.localBaseUrlInput) {
    ...
    const selectedProviderBaseUrl = provider === "auto"
      ? ""
      : defaultBaseUrl;
...
function hydrateSettings(force = false) {
  ...
  setValueIfIdle(elements.agentProfileSelect, settings.agentProfile || "default");
  ...
  setCheckedIfPresent(elements.dreamPetEnabledInput, settings.dreamPetEnabled !== false);
  ...
  setValueIfIdle(elements.dreamPetVoiceInput, String(settings.dreamPetVoiceName || ""));
  applyDreamPetSettings(settings);
...
function renderHeader() {
  ...
  const preview = shortText(lastPreview(chat) || `Conectado ao ${DREAM_DESKTOP_NAME}: ${hermesRouteText || "config Dream Server DESKTOP"}.`, 180);

  const setText = (el, value) => { if (el) el.textContent = value; };
  setText(elements.statusBadge, normalizeStatus(chat?.status));
  setText(elements.providerBadge, DREAM_DESKTOP_NAME);
  ...
  setText(elements.composerProviderHint, `${DREAM_DESKTOP_NAME}: ${hermesRouteText || "config.yaml"}`);
  setText(elements.composerAttachmentHint, state.attachments.length ? `${state.attachments.length} anexo(s)` : "Imagens, PDF e arquivos");
  setText(elements.apiKeyState, `${DREAM_DESKTOP_NAME} usa provider/modelo configurados: ${hermesRouteText || "config.yaml/env do Dream Server DESKTOP"}.`);
  setText(elements.providerSummary, `Runtime: ${DREAM_DESKTOP_NAME}. Provider: ${hermesRouteText || "config.yaml/env"}. Workspace: ${chat?.workspaceRoot || "-"}. Rota: ${chat?.activeRoute?.id || "general-purpose"}.`);
  ...
  if (elements.providerBadgeSettings) {
    elements.providerBadgeSettings.textContent = `${DREAM_DESKTOP_NAME} · ${hermesRoute.label}`;
...
function rememberRuntimeActivity(payload = {}) {
  ...
  const summary = type === "agent_reasoning_delta"
    ...
    : type === "agent_phase_changed"
      ? String(event.summary || event.message || event.phase || "Dream Server DESKTOP pensando").trim()
...
function renderHermesCatalog() {
  ...
  const gatewaysHtml = gateways.length
    ...
    : `<span class="note">Nenhum gateway do ${escapeHtml(DREAM_DESKTOP_NAME)} detectado.</span>`;
...
function renderAppSurface() {
  ...
  if (isKanban) {
    ...
    elements.chatTitle.textContent = "Kanban Dream Server DESKTOP";
    elements.chatSubtitle.textContent = "Planeje, mova e dispare agentes sem sair da base Dream Server DESKTOP.";
    renderKanbanBoard();
  } else if (isTerminals) {
    elements.threadEyebrow.textContent = "Aperant Multiagente";
    elements.chatTitle.textContent = "Agent Terminals";
    elements.chatSubtitle.textContent = "Execucao paralela com terminal_exec, agent_spawn e worktrees pelo Dream Server DESKTOP.";
...
function aperantUtilityMeta(mode = state.appMode) {
  const map = {
    ...
    ideation: {
      ...
      title: "Ideation",
      subtitle: "Converte ideias soltas em cards e subagentes usando Dream Server DESKTOP.",
      action: "Run ideation"
    },
    changelog: {
    ...
    github: {
      ...
      title: "GitHub Issues",
      subtitle: "Cria triagem e correcoes via Dream Server DESKTOP, respeitando git/worktree quando ligado.",
      action: "Sync issues"
    },
    worktrees: {
...
function aperantUtilityObjective(mode, extra = "") {
  ...
  const modePrompts = {
    ...
    ideation: "Turn the user's idea into concrete Dream Server DESKTOP tasks with acceptance criteria and implementation order.",
    changelog: "Draft a changelog from completed work, PR-ready work, and recent task logs. Keep it concise and actionable.",
    context: "Audit project context for Dream Server DESKTOP agents: workspace, provider, routes, constraints, risks, and missing configuration.",
  ...
  return [
    "Use only the Dream Server DESKTOP runtime and the existing project base.",
    `Workspace: ${root}`,
    ...
    "Current task digest:",
    taskDigest,
    "Update/create Kanban tasks through Dream Server DESKTOP tools when useful. Do not bypass Dream Server DESKTOP."
  ].filter(Boolean).join("\n");
}
...
async function runAperantUtilityTask(mode, extra = "") {
  ...
  return await runHermesDirectAction({
    ...
    successMessage: `${meta.title} enviado ao Dream Server DESKTOP.`
  });
}
  ...
  const panels = {
    ...
    ideation: `
      <form class="aperant-idea-form" data-utility-form="ideation">
        <label class="aperant-task-field">
          <span>Idea</span>
          <textarea name="idea" rows="5" placeholder="Describe the feature or experiment. Dream Server DESKTOP will turn it into an execution plan."></textarea>
        </label>
        <button type="submit" class="aperant-new-task-btn">Run ideation with Dream Server DESKTOP</button>
      </form>
      <div class="aperant-utility-grid">
        <section class="aperant-utility-panel">
        ...
        <section class="aperant-utility-panel">
          <div class="view-list-head"><strong>Dream Server DESKTOP routes</strong><span>4</span></div>
    ...
    context: `
      <div class="aperant-context-stack">
        ...
        <section class="aperant-utility-panel">
          <div class="view-list-head"><strong>Dream Server DESKTOP rules</strong><span>${settings.fullAccessMode ? "On" : "Off"}</span></div>
    ...
    github: `
      <div class="aperant-utility-grid">
        <section class="aperant-utility-panel">
          ...
          <p class="aperant-utility-copy">Dream Server DESKTOP can create an issue-triage task, inspect repository context, prepare fixes, and route PR creation through the Kanban flow.</p>
...
function renderAperantSidebar() {
  ...
  if (elements.aperantProviderState) elements.aperantProviderState.textContent = `${DREAM_DESKTOP_NAME} ${provider}`;
            ...
            ? columnTasks.map((task) => {
                ...
                <article class="kanban-card" draggable="true" data-task-card-id="${escapeHtml(task.id)}">
                  ...
                  <div class="kanban-execution-row">
                    ...
                    <strong>${escapeHtml(shortText(executionMessage || latestLog?.content || "Dream Server DESKTOP state machine", 90))}</strong>
                  </div>
...
async function moveKanbanTask(taskId, status) {
  ...
  return await runHermesDirectAction({
    ...
    successMessage: "Card atualizado pelo Dream Server DESKTOP."
  });
}
...
async function startKanbanTask(taskId) {
  ...
  return await runHermesDirectAction({
    ...
    successMessage: "Task enviada ao Dream Server DESKTOP."
  }).finally(() => {
...
async function stopKanbanTask(taskId) {
  ...
  return await runHermesDirectAction({
    ...
    successMessage: "Execucao interrompida pelo Dream Server DESKTOP."
  });
}
...
async function recoverKanbanTask(taskId) {
  ...
  return await runHermesDirectAction({
    ...
    successMessage: "Recuperacao enviada ao Dream Server DESKTOP."
  });
}
...
async function cleanupKanbanTask(taskId) {
  ...
  return await runHermesDirectAction({
    ...
    successMessage: "Cleanup de worktree enviado ao Dream Server DESKTOP."
  });
}
...
async function createKanbanTaskPr(taskId) {
  ...
  return await runHermesDirectAction({
    ...
    successMessage: "Fluxo de PR enviado ao Dream Server DESKTOP."
  });
}
...
async function openHermesTerminal(session = nextTerminalSlot()) {
  ...
  return await runHermesDirectAction(action, {
    successMessage: `Terminal ${session} aberto com Dream Server DESKTOP CLI.`
  });
}
...
function renderMultiAgentDeck() {
  ...
  if (state.renderCache.multiAgents !== agentSignature) {
    ...
    elements.multiAgentList.innerHTML = [
      !fullAccessMode
    ? `<div class="agent-warning-card"><strong>Acesso total desligado</strong><span>Spawn de subagentes e terminais fica bloqueado ate habilitar em Settings > Dream Server DESKTOP.</span></div>`
        : "",
      agents.length
        ? agents.map((agent) => `
          ...
          `).join("")
        : `<div class="kanban-empty"><span class="kanban-empty-icon">+</span><strong>No agents yet</strong><small>Spawn one from the Dream Server DESKTOP panel below.</small></div>`
    ].join("");
  }
          ...
          <article class="multiagent-card terminal-panel terminal-slot-empty is-stopped">
            <div class="terminal-panel-head">
              <div class="terminal-panel-title">
                ...
                <span class="terminal-claude-badge">${escapeHtml(DREAM_DESKTOP_NAME)}</span>
              </div>
            ...
            <div class="terminal-panel-body terminal-empty-body">
              ...
              <p>Opens a CMD-backed Dream Server DESKTOP CLI session.</p>
            </div>
          </article>
        `;
      }
    ...
    elements.settingsTabs.querySelectorAll(".d-tab").forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        if (elements.settingsPaneEyebrow) {
          const eyebrows = {
            general: "Runtime Dream Server DESKTOP",
...
function scheduleCodeShaderSettingsSave() {
  if (!window.dreamDesktop?.saveSettings) {
    return;
  }
  ...
  codeShaderSettingsSaveTimer = window.setTimeout(async () => {
    try {
      const savedApp = await window.dreamDesktop.saveSettings(collectSettingsPayload());
...
async function closeSettingsModal(options = {}) {
  ...
  if (persist && state.app && window.dreamDesktop?.saveSettings) {
    try {
      state.app = await window.dreamDesktop.saveSettings(collectSettingsPayload());
    } catch (error) {
...
function renderAll(options = {}) {
  ...
  updateDreamPetFromAppState(Boolean(options.forceSettings));
  scheduleAutoRun();
...
function dreamPetSettings(settings = state.app?.settings || {}) {
  return {
    ...
    voiceName: String(settings.dreamPetVoiceName || ""),
    externalOverlay: Boolean(state.app?.dreamPetExternalOverlay)
  };
}
...
function preferredDreamPetVoiceName(settings = state.app?.settings || {}) {
  const configured = String(settings.dreamPetVoiceName || "").trim();
  const voices = dreamPetVoices();
  if (configured && voices.some((voice) => voice.name === configured)) {
    return configured;
  }
  const feminineVoice = /maria|francisca|luciana|helia|zira|jenny|aria|female|feminina/i;
  const preferred = voices.find((voice) => /pt[-_]?br/i.test(voice.lang || "") && feminineVoice.test(voice.name || "")) ||
    voices.find((voice) => feminineVoice.test(voice.name || ""));
  return preferred?.name || "";
}

function selectedDreamPetVoice(name = "") {
  const voices = dreamPetVoices();
  const configured = String(name || "").trim();
  if (configured) {
    return voices.find((voice) => voice.name === configured) || null;
  }
  const autoName = preferredDreamPetVoiceName();
  return autoName ? voices.find((voice) => voice.name === autoName) || null : null;
}
...
function hydrateDreamPetVoiceOptions(settings = state.app?.settings || {}) {
  ...
  if (select.dataset.voiceSignature !== signature) {
    select.innerHTML = `<option value="">Auto: voz feminina pt-BR</option>` + voices
  ...
  setValueIfIdle(select, voices.some((voice) => voice.name === selected) ? selected : "");
}

function dreamPetDefaultPosition() {
  const width = elements.dreamPetOverlay?.offsetWidth || 172;
  const height = elements.dreamPetOverlay?.offsetHeight || 188;
  return {
    x: Math.max(14, window.innerWidth - width - 26),
    y: Math.max(46, window.innerHeight - height - 30)
  };
}

function clampDreamPetPosition(x, y) {
  const overlay = elements.dreamPetOverlay;
  const width = overlay?.offsetWidth || 172;
  const height = overlay?.offsetHeight || 188;
  const pad = 10;
  return {
    x: Math.min(Math.max(pad, Number(x) || pad), Math.max(pad, window.innerWidth - width - pad)),
    y: Math.min(Math.max(pad, Number(y) || pad), Math.max(pad, window.innerHeight - height - pad))
  };
}

function readDreamPetPosition() {
  try {
    const saved = JSON.parse(localStorage.getItem("dream.pet.position") || "null");
    if (saved && Number.isFinite(saved.x) && Number.isFinite(saved.y)) {
      return clampDreamPetPosition(saved.x, saved.y);
    }
  } catch {
    // Ignore malformed local position and fall back to the designed corner.
  }
  return dreamPetDefaultPosition();
}

function setDreamPetPosition(x, y, persist = true) {
  const overlay = elements.dreamPetOverlay;
  if (!overlay) {
    return;
  }
  const next = clampDreamPetPosition(x, y);
  overlay.style.left = `${Math.round(next.x)}px`;
  overlay.style.top = `${Math.round(next.y)}px`;
  if (persist) {
    localStorage.setItem("dream.pet.position", JSON.stringify(next));
  }
}

function resetDreamPetPosition() {
  const next = dreamPetDefaultPosition();
  setDreamPetPosition(next.x, next.y, true);
  setDreamPetMode("waving");
  speakDreamPet("Voltei para o canto principal.", { force: true, voice: false });
}

function randomDreamPetLine(mode) {
  const lines = DREAM_PET_LINES[mode] || DREAM_PET_LINES.idle;
  const line = lines[Math.floor(Math.random() * lines.length)] || "Dreamserver online.";
  if (line === state.pet.lastLine && lines.length > 1) {
    return lines[(lines.indexOf(line) + 1) % lines.length];
  }
  return line;
}
...
function dreamPetMood() {
  ...
  if (state.stopping || /fail|error|erro|crash|timeout/.test(status)) {
    return "failed";
  }
  if (state.busy || status === "running" || status === "streaming") {
    return "running";
  }
  if (!elements.settingsModal?.hidden || state.appMode === "kanban" || state.appMode === "terminals") {
    return "review";
  }
  if (state.appMode === "home") {
    return "idle";
  }
  if (chat && visibleEntries(chat).length > 0) {
    return "waiting";
  }
  return "waving";
}

function renderDreamPetFrame() {
  const sprite = elements.dreamPetSprite;
  if (!sprite) {
    return;
  }
  const row = DREAM_PET_ROWS[state.pet.mode] || DREAM_PET_ROWS.idle;
  const frame = state.pet.frame % row.frames;
  sprite.style.backgroundPosition = `-${frame * DREAM_PET_FRAME.width}px -${row.row * DREAM_PET_FRAME.height}px`;
}

function setDreamPetMode(mode) {
  const nextMode = DREAM_PET_ROWS[mode] ? mode : "idle";
  if (state.pet.mode === nextMode && state.pet.animationTimer) {
    return;
  }
  state.pet.mode = nextMode;
  state.pet.frame = 0;
  elements.dreamPetOverlay?.setAttribute("data-pet-state", nextMode);
  renderDreamPetFrame();
  if (state.pet.animationTimer) {
    clearInterval(state.pet.animationTimer);
  }
  const row = DREAM_PET_ROWS[nextMode] || DREAM_PET_ROWS.idle;
  state.pet.animationTimer = setInterval(() => {
    state.pet.frame = (state.pet.frame + 1) % row.frames;
    renderDreamPetFrame();
  }, Math.max(90, Math.round(1000 / row.fps)));
}

function speakDreamPet(line, options = {}) {
  const text = String(line || "").trim();
  if (!text || !elements.dreamPetBubble) {
    return;
  }
  state.pet.lastLine = text;
  elements.dreamPetBubble.hidden = !state.pet.bubbleEnabled;
  if (state.pet.bubbleEnabled) {
    elements.dreamPetBubble.textContent = text;
    elements.dreamPetBubble.style.animation = "none";
    void elements.dreamPetBubble.offsetHeight;
    elements.dreamPetBubble.style.animation = "";
  }
  const canSpeak = state.pet.voiceEnabled && options.voice !== false && "speechSynthesis" in window;
  const now = Date.now();
  if (!canSpeak || (!options.force && now - state.pet.lastSpeechAt < 6500)) {
    return;
  }
  try {
    const utterance = new SpeechSynthesisUtterance(text);
    utterance.lang = effectiveLocale() || "pt-BR";
    utterance.rate = 1.03;
    utterance.pitch = 1.22;
    utterance.volume = 0.72;
    const voice = selectedDreamPetVoice(state.pet.voiceName);
    if (!voice) {
      return;
    }
    ...
    state.pet.lastSpeechAt = now;
  } catch {
    // Speech synthesis is optional; the bubble is the reliable channel.
  }
}

function applyDreamPetSettings(settings = state.app?.settings || {}) {
  const petSettings = dreamPetSettings(settings);
  state.pet.enabled = petSettings.enabled && !petSettings.externalOverlay;
  state.pet.bubbleEnabled = petSettings.bubbleEnabled;
  state.pet.voiceEnabled = petSettings.voiceEnabled;
  state.pet.voiceName = String(settings.dreamPetVoiceName || "").trim();
  setCheckedIfPresent(elements.dreamPetEnabledInput, petSettings.enabled);
  setCheckedIfPresent(elements.dreamPetBubbleEnabledInput, petSettings.bubbleEnabled);
  setCheckedIfPresent(elements.dreamPetVoiceEnabledInput, petSettings.voiceEnabled);
  hydrateDreamPetVoiceOptions(settings);
  if (!elements.dreamPetOverlay) {
    return;
  }
  elements.dreamPetBubble.hidden = !petSettings.bubbleEnabled;
  elements.dreamPetOverlay.hidden = !state.pet.enabled;
  ...
  if (!elements.dreamPetOverlay.style.left || !elements.dreamPetOverlay.style.top) {
    const position = readDreamPetPosition();
    setDreamPetPosition(position.x, position.y, false);
  } else {
    const rect = elements.dreamPetOverlay.getBoundingClientRect();
    setDreamPetPosition(rect.left, rect.top, false);
  }
  setDreamPetMode(state.pet.mode || "idle");
}

function updateDreamPetFromAppState(force = false) {
  applyDreamPetSettings(state.app?.settings || {});
  if (!state.pet.enabled) {
    return;
  }
  const mood = dreamPetMood();
  setDreamPetMode(mood);
  if (elements.dreamPetStatusText) {
    elements.dreamPetStatusText.textContent = ({
      idle: "online",
      waiting: "pronta",
      running: "rodando",
      review: "revisao",
  ...
  if (force || mood !== state.pet.lastMood) {
    state.pet.lastMood = mood;
    speakDreamPet(randomDreamPetLine(mood), { force });
  }
}

function initDreamPet() {
  const overlay = elements.dreamPetOverlay;
  const handle = elements.dreamPetHandle;
  if (!overlay || !handle) {
    return;
  }
  const position = readDreamPetPosition();
  ...
  handle.addEventListener("pointermove", (event) => {
    ...
    setDreamPetPosition(drag.originX + dx, drag.originY + dy, false);
  });
  const finishDrag = (event) => {
    ...
    const rect = overlay.getBoundingClientRect();
    setDreamPetPosition(rect.left, rect.top, true);
    overlay.classList.remove("is-dragging");
    handle.releasePointerCapture?.(event.pointerId);
    state.pet.drag = null;
    if (drag.moved) {
      speakDreamPet("Fico bem aqui.", { force: true, voice: false });
    } else {
      setDreamPetMode("waving");
      speakDreamPet(randomDreamPetLine("waving"), { force: true });
      setTimeout(() => updateDreamPetFromAppState(false), 1200);
    }
  };
  ...
  handle.addEventListener("keydown", (event) => {
    ...
    event.preventDefault();
    setDreamPetPosition(rect.left + move[0], rect.top + move[1], true);
  });
  window.addEventListener("resize", () => {
    const rect = overlay.getBoundingClientRect();
    setDreamPetPosition(rect.left, rect.top, true);
  ...
  elements.dreamPetWakeButton?.addEventListener("click", async () => {
    ...
    try {
      if (window.dreamDesktop?.saveSettings) {
        state.app = await window.dreamDesktop.saveSettings(payload);
      }
      if (window.dreamDesktop?.wakeDreamPet) {
        await window.dreamDesktop.wakeDreamPet();
      } else {
        applyDreamPetSettings(payload);
        setDreamPetMode("waving");
        speakDreamPet(randomDreamPetLine("waving"), { force: true });
        setTimeout(() => updateDreamPetFromAppState(false), 1400);
      }
      renderAll({ forceSettings: true });
    } catch (error) {
  ...
  elements.dreamPetResetButton?.addEventListener("click", async () => {
    if (window.dreamDesktop?.resetDreamPet) {
      await window.dreamDesktop.resetDreamPet();
      showToast("Dreamserver reposicionada.");
      return;
    }
    resetDreamPetPosition();
  });
  elements.dreamPetEnabledInput?.addEventListener("change", async () => {
    const payload = collectSettingsPayload();
    applyDreamPetSettings(payload);
    try {
      if (window.dreamDesktop?.saveSettings) {
      ...
      showToast(elements.dreamPetEnabledInput.checked ? "Pet Dreamserver ativado." : "Pet Dreamserver desativado.");
    } catch (error) {
      showToast(error.message || "Nao consegui salvar o estado do pet.");
    }
  });
  elements.dreamPetVoiceEnabledInput?.addEventListener("change", async () => {
    const payload = collectSettingsPayload();
    applyDreamPetSettings(payload);
    try {
      if (window.dreamDesktop?.saveSettings) {
      ...
      showToast(elements.dreamPetVoiceEnabledInput.checked ? "Som da Dreamserver ligado." : "Som da Dreamserver desligado.");
    } catch (error) {
      showToast(error.message || "Nao consegui salvar o som do pet.");
    }
  });
  elements.dreamPetBubbleEnabledInput?.addEventListener("change", async () => {
    const payload = collectSettingsPayload();
    applyDreamPetSettings(payload);
    try {
      if (window.dreamDesktop?.saveSettings) {
      ...
      showToast(elements.dreamPetBubbleEnabledInput.checked ? "Balao da Dreamserver ligado." : "Balao da Dreamserver desligado.");
    } catch (error) {
      showToast(error.message || "Nao consegui salvar o balao do pet.");
    }
  });
  elements.dreamPetVoiceInput?.addEventListener("change", async () => {
    const payload = collectSettingsPayload();
    applyDreamPetSettings(payload);
    try {
      if (window.dreamDesktop?.saveSettings) {
      ...
      showToast("Voz da Dreamserver atualizada.");
    } catch (error) {
      showToast(error.message || "Nao consegui salvar a voz do pet.");
    }
  });
...
function scheduleRuntimeRefresh(event) {
  ...
  state.refreshTimer = setTimeout(async () => {
    try {
      state.app = await window.dreamDesktop.loadState();
      renderAll();
...
async function runAction(actionKey) {
  ...
  renderAll();
  try {
    const result = await window.dreamDesktop.runDesktopAction({
      chatId: chat.id,
      actionKey,
      action
    });
...
async function ensureActionChat() {
  ...
  state.app = await window.dreamDesktop.createChat();
  chat = currentChat();
  if (!chat) {
    throw new Error("Nao consegui criar uma sessao para executar a action Dream Server DESKTOP.");
  }
  return chat;
}
...
async function runHermesDirectAction(action, options = {}) {
  ...
  try {
    ...
    showToast(error.message || "Falha ao executar action Dream Server DESKTOP.");
    return null;
  } finally {
    state.runningActions.delete(actionKey);
    renderAll();
...
function collectSettingsPayload(providerMode = activeProvider()) {
  ...
  const executionProvider = executionProviderForSettings({
    ...settings,
    hermesProvider,
    localBaseUrl,
    localModel
  ...
  return {
    ...
    agentProfile: fieldValue(elements.agentProfileSelect, settings.agentProfile || "default"),
    ...
    dreamPetEnabled: fieldChecked(elements.dreamPetEnabledInput, settings.dreamPetEnabled !== false),
...
async function switchProviderMode(mode) {
  const settings = state.app?.settings || {};
  const currentProvider = normalizeHermesProvider(settings.hermesProvider || "custom");
  const currentIsCloud = !routeUsesLocalEndpoint(settings);
  const hermesProvider = mode === "cloud" ? (currentIsCloud ? currentProvider : "auto") : "custom";
  if (elements.hermesProviderInput) {
    elements.hermesProviderInput.value = hermesProvider;
  }
  const providerMode = mode === "cloud" ? "cloud" : "local";
  state.app = await window.dreamDesktop.saveSettings({
    ...collectSettingsPayload(providerMode),
    hermesProvider,
    providerMode
  });
  if (currentChat()) {
    state.app = await window.dreamDesktop.setChatProvider({
...
async function sendMessage() {
  ...
  try {
    ...
    const response = await window.dreamDesktop.sendMessage({
      chatId: chat.id,
      text,
      attachmentPaths
    });
...
async function stopActiveChat() {
  ...
  renderAll();
  try {
    const response = await window.dreamDesktop.stopChat(chat.id);
    state.app = response.state;
...
async function stopAllLocalActivity() {
  ...
  renderAll();
  try {
    const response = await window.dreamDesktop.stopAllLocalActivity();
    state.app = response.state;
...
async function stopBackgroundJob(jobId) {
  ...
  try {
    const response = await window.dreamDesktop.stopBackgroundJob(jobId);
    state.app = response.state;
...
async function closeTerminalSession(sessionId) {
  ...
  await runHermesDirectAction({
    ...
    successMessage: "Terminal fechado pelo Dream Server DESKTOP."
  });
}

async function refreshLocalModels() {
  try {
    state.localModels = await window.dreamDesktop.listLocalModels();
...
async function startLocalLlama() {
  try {
    state.app = await window.dreamDesktop.saveSettings(collectSettingsPayload("local"));
    state.app = await window.dreamDesktop.startLocalLlama({ forceRestart: false });
    renderAll({ forceSettings: true });
...
async function stopLocalLlama() {
  try {
    state.app = await window.dreamDesktop.stopLocalLlama();
    renderAll({ forceSettings: true });
...
async function init() {
  ...
  window.dreamDesktop.onRuntimeEvent((event) => {
    updateHomeTokenTelemetryFromRuntimeEvent(event);
    scheduleRuntimeRefresh(event);
  });
  window.dreamDesktop.onPreviewHarnessCommand?.(async (payload) => {
    try {
      const result = await runPreviewHarnessCommand(payload?.command || {});
      await window.dreamDesktop.completePreviewHarnessCommand({
        id: payload?.id,
        ok: true,
        result
      });
  ...
  elements.aperantUtilityView?.addEventListener("submit", async (event) => {
    ...
    if (!idea) {
      showToast("Informe uma ideia para o Dream Server DESKTOP transformar em plano.");
      return;
    }
  ...
  elements.chatList.addEventListener("click", async (event) => {
    ...
    if (deleteButton) {
      state.app = await window.dreamDesktop.deleteChat(deleteButton.dataset.deleteChat);
      renderAll();
      return;
    }
    ...
    if (row) {
      state.app = await window.dreamDesktop.selectChat(row.dataset.chatId);
      renderAll();
    }
  });
  ...
  elements.kanbanTaskForm?.addEventListener("submit", async (event) => {
    ...
    const result = await runHermesDirectAction({
      ...
      successMessage: "Card criado pelo Dream Server DESKTOP."
    });
    if (result) {
  ...
  elements.agentSpawnForm?.addEventListener("submit", async (event) => {
    ...
    const result = await runHermesDirectAction(action, {
      successMessage: "Subagente iniciado pelo Dream Server DESKTOP."
    });
    if (result) {
  ...
  elements.multiAgentList?.addEventListener("click", async (event) => {
    ...
    if (stopButton && !stopButton.disabled) {
      await runHermesDirectAction({
        ...
        successMessage: "Subagente interrompido pelo Dream Server DESKTOP."
      });
      return;
    }
    ...
    if (openChatButton) {
      state.app = await window.dreamDesktop.selectChat(openChatButton.dataset.openAgentChat);
      setAppMode("chat");
    }
  });
  ...
  elements.projectFeed.addEventListener("click", (event) => {
    ...
    if (pathButton) {
      window.dreamDesktop.openExternal(fileUrl(pathButton.dataset.projectPath));
      return;
    }
    const urlButton = event.target.closest("[data-project-url]");
    if (urlButton) {
      window.dreamDesktop.openExternal(urlButton.dataset.projectUrl);
    }
  });
  ...
  elements.previewOpenButton.addEventListener("click", () => {
    ...
    if (url) {
      window.dreamDesktop.openExternal(url);
    }
  });
  elements.previewDesktopButton?.addEventListener("click", async () => {
    ...
    renderWorkbenchPanel();
    try {
      const response = await window.dreamDesktop.setPreviewMode({
        mode: "desktop",
        source: "ui"
      });
      state.app = response.state;
  ...
  elements.codeSurface.addEventListener("click", async (event) => {
    ...
    const chat = currentChat();
    try {
      const response = await window.dreamDesktop.saveCodeFile({
        chatId: chat?.id,
  ...
  elements.attachButton.addEventListener("click", async () => {
    const picked = await window.dreamDesktop.pickAttachments();
  ...
  elements.pickBackgroundButton?.addEventListener("click", async () => {
    try {
      const picked = await window.dreamDesktop.pickBackgroundMedia();
      if (!picked?.path) {
        return;
      }
      const payload = {
  ...
  elements.clearApiKeyButton?.addEventListener("click", async () => {
    state.app = await window.dreamDesktop.clearApiKey();

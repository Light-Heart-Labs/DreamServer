...
In a normal DreamServer install, `TOKEN_SPY_API_KEY` is either written to `.env`
or generated by Token Spy on first start. When Token Spy generates it, the key is
persisted at `data/token-spy/token-spy-api-key.txt` on the host
(`/app/data/token-spy-api-key.txt` inside the container). Use that value when the
Token Spy dashboard asks for an API key, and do not paste it into public logs.

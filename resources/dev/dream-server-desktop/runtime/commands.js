...
function slashHelp() {
  return [
    ...
    "/provider [manus|openrouter|openai|anthropic|gemini|kimi|nvidia|custom|auto] - troca o provider Hermes ativo",
    "/model [nome] - troca o modelo Hermes ativo",
...
async function runSlashCommand(runtime, chat, input) {
  ...
  if (command === "/provider") {
    if (!value) {
      return `Provider Hermes atual: ${hermesRouteSummary(runtime.state.settings)}.`;
    }
    const provider = normalizeProvider(value) || "custom";
    const providerMode = provider === "manus" ? "cloud" : "local";
    runtime.updateSettings({ hermesProvider: provider, providerMode });
    runtime.setChatProvider(chat.id, providerMode);
    return `Provider Hermes ativo: ${hermesRouteSummary(runtime.state.settings)}.`;
  }

  if (command === "/model") {
    if (!value) {
      return `Modelo Hermes atual: ${hermesRouteSummary(runtime.state.settings)}.`;
    }
    runtime.updateSettings({ localModel: value });
    return `Modelo Hermes atualizado: ${hermesRouteSummary(runtime.state.settings)}.`;
  }
  ...
  if (command === "/doctor") {
    ...
    return [
      ...
      `- rota Hermes: ${hermesRouteSummary(settings)}`,

...
const { defaultTerminalShellName, executeTool, getTerminalSessionSnapshots } = require("./tools");

function testShell() {
  return defaultTerminalShellName();
}

function shellOutputCommand(text) {
  const shell = testShell();
  if (shell === "cmd") {
    return `echo ${text}`;
  }
  return ["powershell", "pwsh"].includes(shell)
    ? `Write-Output ${JSON.stringify(text)}`
    : `printf '%s\\n' ${JSON.stringify(text)}`;
}

function shellSleepCommand(seconds) {
  const shell = testShell();
  if (shell === "cmd") {
    return `ping -n ${Number(seconds) + 1} 127.0.0.1 > nul`;
  }
  return ["powershell", "pwsh"].includes(shell)
    ...
    const result = await executeTool(
      {
        type: "terminal_exec",
        session: "state-test",
        command: shellOutputCommand("dream-terminal-ok"),
        timeoutMs: 10000
      },
      {
        workspaceRoot: root,

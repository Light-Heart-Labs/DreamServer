...
const { AgentRuntime, _test: agentRuntimeTest } = require("./agent-runtime");
...
async function testTaskRecordDefaultsWorkspaceRoot() {
  const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dream-kanban-root-"));
  const runtime = new DreamRuntime({
    workspaceRoot,
    disableTaskScheduler: true,
    disableTaskTerminals: true
  });
  const task = runtime.createTaskRecord({
    title: "Task sem workspace explicito",
    objective: "Garantir que o agente use o workspace principal",
    status: "backlog"
  });

  assert.equal(path.resolve(task.workspaceRoot), path.resolve(workspaceRoot));
}
...
async function testArchivedTaskCanBeDeletedFromKanban() {
  ...
  const task = runtime.createTaskRecord({
    title: "Archive me",
    objective: "Validate archived delete flow",
    status: "done"
  });

  const archivedState = await runtime.runSuggestedAction({
    chatId: chat.id,
    actionKey: `manual-${crypto.randomUUID()}:0`,
    action: {
      type: "task_update",
  ...
  assert.equal(archivedState.tasks.find((entry) => entry.id === task.id)?.status, "archived");

  const deletedState = await runtime.runSuggestedAction({
    chatId: chat.id,
    actionKey: `manual-${crypto.randomUUID()}:1`,
    action: {
      type: "task_delete",
      id: task.id
    }
  });
  assert.equal(deletedState.tasks.some((entry) => entry.id === task.id), false);
}
...
async function testHermesToolEventsPopulateWorkbenchArtifacts() {
  ...
  agent._ingestHermesToolEvents(chat, runState, [
    ...
    },
    {
      ...
      result: JSON.stringify({ status: "ok", result: "Snapshot captured from active Workbench preview." })
    },
    {
      type: "tool_complete",
      id: "tool-6",
  ...
  assert.equal(chat.localEvents.length, 6);
  ...
  assert.equal(chat.localEvents[4].action.type, "browser_control");
  assert.equal(chat.localEvents[4].action.operation, "control");
  assert.deepEqual(chat.localEvents[4].action.steps, [{ type: "snapshot" }]);
  assert.equal(chat.localEvents[5].action.type, "hermes_tool");
  assert.equal(chat.localEvents[5].action.tool, "made_up_tool");
...
async function testSettingsPartialUpdatePreservesBooleansAndHermesDefaults() {
  const runtime = new DreamRuntime({
  ...
  runtime.updateSettings({ agentProfile: "manus-1.6" });

  assert.equal(runtime.state.settings.fullAccessMode, true);
  assert.equal(runtime.state.settings.desktopBridgeEnabled, true);
...
async function testCloudHermesRouteDoesNotAutostartLocalLlama() {
  assert.equal(
    agentRuntimeTest.hermesRouteNeedsManagedLocalLlama({
      hermesProvider: "nvidia",
      localBaseUrl: "http://127.0.0.1:11435/v1",
      localModel: "Qwen3.5-9B-Q4_K_M",
      localLlamaEnabled: true,
      localLlamaAutoStart: true
    }),
    false
  );
  assert.equal(
...
async function main() {
  const tests = [
    ...
    testSettingsPartialUpdatePreservesBooleansAndHermesDefaults,
    testCloudHermesRouteDoesNotAutostartLocalLlama
  ];

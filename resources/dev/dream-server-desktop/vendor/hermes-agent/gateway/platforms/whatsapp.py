...
def _resolve_executable(name: str, env_keys: tuple[str, ...] = ()) -> str:
    for key in env_keys:
        value = os.getenv(key)
        if value:
            return value
...
def _node_command() -> str:
    return _resolve_executable("node", ("DREAM_DESKTOP_NODE", "HERMES_NODE_COMMAND"))


def _npm_command() -> str:
    return _resolve_executable("npm", ("DREAM_DESKTOP_NPM", "HERMES_NPM_COMMAND"))


def _install_timeout_seconds() -> int:
    try:
        return max(60, int(os.getenv("WHATSAPP_BRIDGE_INSTALL_TIMEOUT", "300")))
    except ValueError:
        return 300
        ...
        try:
            ...
            if not (bridge_dir / "node_modules").exists():
                ...
                try:
                    ...
                    print(f"[{self.name}] Failed to install dependencies with {npm_command}: {e}")
                    return False
            ...
            if data.get("status") != "connected":
                bridge_status = data.get("status", "unknown")
                print(f"[{self.name}] Bridge HTTP ready (status: {bridge_status})")
                print(f"[{self.name}] Pairing/QR log: {self._bridge_log}")

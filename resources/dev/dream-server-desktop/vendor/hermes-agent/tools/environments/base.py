...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        """
        initial_cwd = self._quote_cwd_for_bash(self.cwd)
        # Full capture: env vars, functions (filtered), aliases, shell options.
        bootstrap = (
            f"cd {initial_cwd} || exit 126\n"
    ...
    @staticmethod
    def _quote_cwd_for_bash(cwd: str) -> str:
        value = str(cwd or ".")
        return value if value == "~" or value.startswith("~/") else shlex.quote(value)
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        quoted_cwd = self._quote_cwd_for_bash(cwd)

...
_dream_browser_last_urls: Dict[str, str] = {}
_dream_browser_ref_selectors: Dict[str, Dict[str, str]] = {}
_dream_browser_ref_labels: Dict[str, Dict[str, str]] = {}
...
BROWSER_TOOL_SCHEMAS = [
    ...
    {
        "name": "browser_click",
        "description": "Click on an element identified by its ref ID from the latest snapshot (e.g., '@e5'). If the snapshot shows a visible label, include it in label to guard against stale or shifted refs. Requires browser_navigate or browser_snapshot first. Do not use this for chessboard squares on Lichess; use browser_chess_state and browser_chess_move.",
        "parameters": {
            "type": "object",
            "properties": {
                "ref": {
                ...
                "label": {
                    "type": "string",
                    "description": "Optional exact visible label/accessibility name for the target element. Use this with ref when available, e.g. 'Jogar contra o computador'."
                }
            },
            "required": ["ref"]
        }
    ...
    {
        "name": "browser_chess_state",
        "description": "Read the active Workbench chess board as structured text/DOM state: orientation, controlled color, piece placement, and pieces by square. This does not choose a move. Use it before browser_chess_move when playing chess with a text-only model.",
        "parameters": {
            "type": "object",
            "properties": {},
            "required": []
    ...
    },
    {
        ...
        "description": "Move a chess piece on the active Workbench board by algebraic squares, e.g. from_square='e2', to_square='e4'. The model must decide the move from browser_chess_state/browser_snapshot; this tool only executes and verifies the move. It targets the live cg-board DOM and does not use screenshot coordinates, image/vision support, or generic page clicks.",
        "parameters": {
            "type": "object",
            "properties": {
                "from_square": {
                ...
                "promotion": {
                    "type": "string",
                    "description": "Optional promotion piece: queen, rook, bishop, or knight"
                }
            },
    ...
    {
        "name": "browser_chess_wait_turn",
        "description": "Wait on the active Workbench chess board until sideToMove matches the controlled/playAs side. Use this immediately after a successful browser_chess_move when the game is still active, then choose and execute the next move. Do not final-answer with 'waiting' while a chess game is active.",
        "parameters": {
            "type": "object",
            "properties": {
                "timeout_seconds": {
                    "type": "number",
                    "description": "Maximum seconds to wait for the opponent to move. Defaults to 30."
                }
            },
            "required": []
        }
    ...
    {
        "name": "browser_click_square",
        "description": "Click one square on the active chess board by algebraic coordinate, e.g. 'e4'. Prefer browser_chess_move for complete chess moves.",
        "parameters": {
            "type": "object",
            "properties": {
                "square": {
                    "type": "string",
                    "description": "Chess square in algebraic notation, e.g. e4"
                }
            },
            "required": ["square"]
        }
    ...
    {
        "name": "browser_vision",
        "description": "Explicit visual screenshot analysis only. Do not use this for normal page interaction, games, chess, buttons, forms, or Workbench automation; use browser_snapshot, browser_click, browser_type, browser_press, browser_scroll, browser_chess_state, or browser_chess_move instead. In Dream Desktop, text/DOM mode is preferred and this returns a DOM snapshot instead of requiring image/vision support unless a true visual capture is explicitly needed.",
        "parameters": {
            "type": "object",
            "properties": {
                "question": {
...
def _dream_desktop_available() -> bool:
    """Return True when Hermes is embedded in Dream Desktop with the bridge available."""
    if os.environ.get("DREAM_DESKTOP_INTEGRATION_ENABLED", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return False

    if os.environ.get("DREAM_DESKTOP_ROOT"):
        return True

    # Vendored layout: <project>/vendor/hermes-agent/tools/browser_tool.py
    try:
        project_root = Path(__file__).resolve().parents[3]
        return (project_root / "bin" / "dream-desktop-tool.js").exists()
    except Exception:
        return False


def _force_dream_browser_fallback() -> bool:
    backend = os.environ.get("DREAM_BROWSER_BACKEND", "").strip().lower()
    return backend in {"dream", "desktop", "workbench"}


def _should_use_dream_browser_fallback() -> bool:
    """Use Dream's Playwright bridge only when no richer Hermes browser backend is active."""
    if _force_dream_browser_fallback():
        return _dream_desktop_available()
    if _is_camofox_mode() or _get_cdp_override() or _get_cloud_provider() is not None:
        return False
    return _dream_desktop_available()


def _extract_dream_browser_fields(text: str, fallback_url: str = "") -> Dict[str, str]:
    fields: Dict[str, str] = {
        "url": fallback_url,
        "title": "",
        "snapshot": "",
    ...
    screenshot_path = _extract_screenshot_path_from_text(text)
    if screenshot_path:
        fields["path"] = screenshot_path

    preview_match = re.search(
        r"Text preview:\n(?P<preview>.*?)(?:\n(?:Console|Page errors|Steps):\n|\Z)",
        text,
        flags=re.DOTALL,
    )
    interactive_match = re.search(
        r"Interactive elements:\n(?P<elements>.*?)(?:\n(?:Text preview|Console|Page errors|Steps):\n|\Z)",
        text,
        flags=re.DOTALL,
    )
    interactive_text = interactive_match.group("elements").strip() if interactive_match else ""
    if preview_match:
        preview_text = preview_match.group("preview").strip()
        fields["snapshot"] = "\n".join(
            part for part in [
                f"Interactive elements:\n{interactive_text}" if interactive_text else "",
                f"Page text:\n{preview_text}" if preview_text else "",
            ] if part
        )
    elif interactive_text:
        fields["snapshot"] = f"Interactive elements:\n{interactive_text}"

    return fields


def _extract_dream_browser_refs(text: str) -> Dict[str, str]:
    refs: Dict[str, str] = {}
    if not text:
        return refs

    for line in text.splitlines():
        match = re.match(r"^(@e\d+)\s+(.+?)\s+::\s+", line.strip())
        if match:
            refs[match.group(1)] = match.group(2).strip()
    return refs


def _run_dream_bridge_action(action: Dict[str, Any]) -> Dict[str, Any]:
    try:
        from tools import dream_desktop_tool
    except Exception as exc:
        return {
            "success": False,
            "error": f"Dream desktop bridge unavailable: {exc}",
        }

    try:
        raw = dream_desktop_tool._run_dream_action(action)
    except Exception as exc:
        return {
            "success": False,
            "error": f"Dream desktop bridge failed: {exc}",
        }
    ...
    try:
        ...
        parsed = {"ok": bool(text), "result": text}
    ...
    if isinstance(result, (dict, list)):
        ...
        result_text = str(result or text)
...
def _parse_dream_bridge_json_result(bridge: Dict[str, Any]) -> Dict[str, Any]:
    text = str((bridge or {}).get("result_text") or "").strip()
    if not text:
        return {}
    try:
        parsed = json.loads(text)
...
def _format_dream_browser_snapshot(payload: Dict[str, Any]) -> str:
    if not isinstance(payload, dict):
        return ""

    lines: List[str] = []
    chess_boards = payload.get("chessBoards") or payload.get("gameBoards") or []
    has_chess_board = bool(chess_boards)
    active_chess_game = any(
        isinstance(board, dict) and bool(board.get("activeGame"))
        for board in chess_boards
    )
    if chess_boards:
        lines.append("Chess boards:")
        for index, board in enumerate(chess_boards[:4], start=1):
            if not isinstance(board, dict):
                continue
            orientation = str(board.get("orientation") or "white").strip()
            controlled_color = str(board.get("controlledColor") or board.get("playAs") or orientation).strip()
            opponent_color = str(board.get("opponentColor") or ("white" if controlled_color == "black" else "black")).strip()
            pieces = board.get("pieces") or []
            piece_count = len(pieces) if isinstance(pieces, list) else 0
            piece_placement = str(board.get("piecePlacement") or "").strip()
            side_to_move = str(board.get("sideToMove") or "").strip()
            agent_turn = board.get("isAgentTurn")
            turn_text = str(board.get("turnText") or "").strip()
            last_move = board.get("lastMoveSquares") if isinstance(board.get("lastMoveSquares"), list) else []
            last_move_text = "-".join(str(square) for square in last_move if square)
            active = " activeGame=true" if board.get("activeGame") else ""
            lines.append(
                f"@board{index} orientation={orientation} controlled={controlled_color} opponent={opponent_color} sideToMove={side_to_move or 'unknown'} agentTurn={agent_turn} turnText={turn_text or 'none'} lastMove={last_move_text or 'none'} pieces={piece_count}{active} "
                "state=browser_chess_state() move=browser_chess_move(from_square,to_square) wait=browser_chess_wait_turn()"
            )
            if piece_placement:
                lines.append(f"fenBoard: {piece_placement}")
                rows = piece_placement.split("/")
                if len(rows) == 8:
                    lines.append("boardRows: " + " | ".join(f"{8 - idx}: {row}" for idx, row in enumerate(rows)))
            position = board.get("position") if isinstance(board.get("position"), dict) else {}
            white = str(position.get("white") or "").strip()
            black = str(position.get("black") or "").strip()
            if white:
                lines.append(f"white: {white}")
            if black:
                lines.append(f"black: {black}")
            moves_san = board.get("moveHistorySan") if isinstance(board.get("moveHistorySan"), list) else []
            moves_san_text = " ".join(str(move) for move in moves_san[-24:] if move)
            if moves_san_text:
                lines.append(f"movesSan: {moves_san_text}")
            lines.append("square note: all piece squares above are algebraic board coordinates from White's perspective, not screen coordinates.")

    elements = payload.get("interactiveElements") or []
    if elements and not active_chess_game:
        lines.append("Interactive elements:")
        for entry in elements[:16 if has_chess_board else 80]:
            if not isinstance(entry, dict):
                continue
            ref = str(entry.get("ref") or "").strip()
            selector = str(entry.get("selector") or entry.get("tag") or "").strip()
            ...
            if entry.get("x") is not None and entry.get("y") is not None:
                xy = f" ({entry.get('x')},{entry.get('y')})"
            if ref:
                lines.append(f"{ref} {selector}{xy} :: {label}")
    elif active_chess_game:
        lines.append("Interactive elements omitted: active chess game detected; use browser_chess_state, browser_chess_move, and browser_chess_wait_turn, not page/menu clicks.")

    landmarks = payload.get("landmarks") or []
    if landmarks and not has_chess_board:
        lines.append("Page landmarks:")
        for entry in landmarks[:30]:
            if not isinstance(entry, dict):
                continue
            ref = str(entry.get("ref") or "").strip()
            selector = str(entry.get("selector") or entry.get("tag") or "").strip()
    ...
    text_preview = str(payload.get("textPreview") or "").strip()
    if text_preview:
        lines.append("Page text:")
        lines.append(text_preview[:120 if active_chess_game else 360 if has_chess_board else 2000])

    return "\n".join(lines).strip()


def _same_browser_url(left: str, right: str) -> bool:
...
def _update_dream_browser_state_from_payload(task_id: str, payload: Dict[str, Any], fallback_url: str = "") -> Dict[str, str]:
    url = str((payload or {}).get("url") or fallback_url or "").strip()
    if url:
        _dream_browser_last_urls[task_id] = url

    refs: Dict[str, str] = {}
    labels: Dict[str, str] = {}
    for group_name in ("interactiveElements", "landmarks"):
        for entry in (payload or {}).get(group_name) or []:
            if not isinstance(entry, dict):
                continue
            ref = str(entry.get("ref") or "").strip()
            selector = str(entry.get("selector") or "").strip()
            label = str(entry.get("label") or entry.get("accessibleName") or entry.get("name") or "").strip()
            if ref:
                refs[ref] = selector
                labels[ref] = label
    if refs:
        _dream_browser_ref_selectors[task_id] = refs
    if labels:
        _dream_browser_ref_labels[task_id] = labels
    return refs


def _parse_dream_browser_payload(bridge: Dict[str, Any], fallback_url: str = "") -> Dict[str, Any]:
    payload = _parse_dream_bridge_json_result(bridge)
    if payload:
        return payload

    fields = _extract_dream_browser_fields(bridge.get("result_text", ""), fallback_url)
    refs = _extract_dream_browser_refs(bridge.get("result_text", ""))
    return {
        "source": "legacy-workbench-preview",
        "url": fields.get("url") or fallback_url,
        "title": fields.get("title", ""),
        "textPreview": fields.get("snapshot", ""),
        "interactiveElements": [
            {"ref": ref, "selector": selector, "label": selector}
            for ref, selector in refs.items()
        ],
        "raw": fields.get("raw", ""),
...
def _rehydrate_dream_browser_state(task_id: str, timeout_ms: int) -> Dict[str, Any]:
    bridge = _run_dream_bridge_action({
    ...
    payload = _parse_dream_browser_payload(bridge)
    if not payload:
        return {}
    _update_dream_browser_state_from_payload(task_id, payload)
    return payload


def _run_dream_browser_command(
    task_id: str,
    command: str,
    args: List[str],
    timeout: Optional[int] = None,
) -> Dict[str, Any]:
    """Fallback for Hermes browser tools using Dream Desktop's browser-harness Workbench bridge."""
    args = args or []
    timeout_ms = max(int(timeout or _get_command_timeout() or 30), 5) * 1000
    current_url = _dream_browser_last_urls.get(task_id, "")
    session_state: Dict[str, Any] = {}
    warning = ""

    def ensure_session_state(force: bool = False) -> str:
        nonlocal current_url, session_state
        needs_refresh = force or not current_url
        if not needs_refresh:
            return current_url
        session_state = _rehydrate_dream_browser_state(task_id, timeout_ms)
        refreshed = str(session_state.get("url") or _dream_browser_last_urls.get(task_id, "") or "").strip()
        if refreshed:
    ...
    if command == "open":
        if not args:
            return {"success": False, "error": "browser open requires a URL"}
        requested_url = str(args[0] or "").strip()
        live_url = ensure_session_state(force=True)
        if _same_browser_url(live_url, requested_url):
            ...
            _dream_browser_last_urls[task_id] = current_url
            action = {
                "type": "browser_harness",
                "command": "goto",
                "url": current_url,
        ...
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then call browser_snapshot again; do not open the OS browser as fallback."}
        action = {
            "type": "browser_harness",
            "command": "snapshot",
            "screenshot": False,
        ...
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then retry the screenshot; do not open the OS browser as fallback."}
        action = {
            "type": "browser_harness",
            "command": "screenshot",
            "screenshot": True,
        ...
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then retry the key press; do not open the OS browser as fallback."}
        key = args[0] if args else ""
        if "lichess.org" in str(current_url or "").lower():
            warning = (
                "browser_press was converted to browser_chess_state on Lichess. "
                "Keyboard letters do not reliably play chess moves here; choose a move from the state "
                "and call browser_chess_move(from_square, to_square)."
            )
            action = {
                "type": "browser_harness",
                "command": "chess_state",
        ...
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then retry the scroll; do not open the OS browser as fallback."}
        ...
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then retry the click/type; do not open the OS browser as fallback."}
        target = args[0] if args else ""
        expected_label = args[1] if len(args) > 1 else ""
        ref = target if target.startswith("@") else ""
        selector = "" if ref else target
        label = expected_label or (_dream_browser_ref_labels.get(task_id, {}).get(ref, "") if ref else target)
        if ref and ref not in _dream_browser_ref_selectors.get(task_id, {}):
            ensure_session_state(force=True)
        if not ref and not selector:
            return {
                "success": False,
                "error": "Dream browser fallback could not resolve the requested browser element. Navigate or snapshot first.",
            }
        action = {
            "type": "browser_harness",
            "command": "click",
    ...
    elif command == "chess_state":
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then retry the chess state read; do not open the OS browser as fallback."}
        action = {
            "type": "browser_harness",
            "command": "chess_state",
            "screenshot": False,
    ...
    elif command == "chess_move":
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then retry the chess move; do not open the OS browser as fallback."}
        from_square = str(args[0] if args else "").strip().lower()
        to_square = str(args[1] if len(args) > 1 else "").strip().lower()
        promotion = str(args[2] if len(args) > 2 else "").strip().lower()
        if not re.fullmatch(r"[a-h][1-8]", from_square) or not re.fullmatch(r"[a-h][1-8]", to_square):
            return {"success": False, "error": "browser_chess_move requires algebraic squares like e2 and e4."}
        chess_move_timeout_ms = max(timeout_ms, 70000)
        action = {
            "type": "browser_harness",
            "command": "chess_move",
            ...
            "promotion": promotion,
            "screenshot": False,
            "timeoutMs": chess_move_timeout_ms,
        }
    elif command == "chess_wait_turn":
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then retry waiting for the chess turn; do not open the OS browser as fallback."}
        wait_timeout_ms = timeout_ms
        if args:
            try:
                wait_timeout_ms = max(800, min(int(float(args[0]) * 1000), 120000))
    ...
    elif command == "click_square":
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then retry the chess square click; do not open the OS browser as fallback."}
        square = str(args[0] if args else "").strip().lower()
        if not re.fullmatch(r"[a-h][1-8]", square):
            return {"success": False, "error": "browser_click_square requires an algebraic square like e4."}
        action = {
            "type": "browser_harness",
        ...
        ref = target if target.startswith("@") else ""
        selector = "" if ref else target
        label = _dream_browser_ref_labels.get(task_id, {}).get(ref, "") if ref else target
        if ref and ref not in _dream_browser_ref_selectors.get(task_id, {}):
            ensure_session_state(force=True)
        if not ref and not selector:
            return {
        ...
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then retry back navigation; do not open the OS browser as fallback."}
        action = {
            "type": "browser_harness",
            "command": "back",
            "screenshot": False,
    ...
    elif command in {"eval", "js"}:
        if not ensure_session_state():
            return {"success": False, "error": "No active Dream Workbench browser URL was reported by the live preview bridge. Open or refresh a URL in the Workbench, then retry JavaScript evaluation; do not open the OS browser as fallback."}
        action = {
            "type": "browser_harness",
            "command": "js",
        ...
        return {
            "success": False,
            "error": (
                f"Dream browser fallback does not support browser command '{command}'. "
                "Install/configure agent-browser or Camofox for the full Hermes browser backend."
            ),
        }

    bridge = _run_dream_bridge_action(action)
    if not bridge.get("success"):
        return {
            "success": False,
            "error": bridge.get("error") or bridge.get("result_text") or "Dream browser bridge failed",
        }

    payload = _parse_dream_browser_payload(bridge, current_url)
    refs = _update_dream_browser_state_from_payload(task_id, payload, current_url)
    snapshot_text = _format_dream_browser_snapshot(payload)
    url = str(payload.get("url") or current_url or "").strip()
    ...
    data: Dict[str, Any] = {
        ...
        "refs": refs,
        "raw": bridge.get("result_text", ""),
    }
    if payload.get("chessBoards"):
        data["chessBoards"] = payload.get("chessBoards")
    if warning:
...
def _run_browser_command(
    ...
    if _force_dream_browser_fallback():
        return _run_dream_browser_command(task_id, command, args, timeout)
    
    # Build the command
    try:
        browser_cmd = _find_agent_browser()
    except FileNotFoundError as e:
        if _should_use_dream_browser_fallback():
            logger.info("agent-browser CLI not found; using Dream browser fallback for '%s'", command)
            return _run_dream_browser_command(task_id, command, args, timeout)
...
def _dream_browser_text_snapshot_for_vision(
    question: str,
    annotate: bool,
    task_id: str,
) -> str:
    """Return Workbench DOM state when a text-only desktop model asks for vision.

    Dream Desktop deliberately keeps Workbench browser automation usable with
    text-only local models.  If Hermes asks for browser_vision in this mode,
    do not attempt PNG capture; that path depends on agent-browser screenshot
    files and breaks local Workbench sessions.  The agent should reason from
    the live DOM/accessibility/chess snapshot instead.
    """
    result = _run_dream_browser_command(task_id, "snapshot", [], timeout=_get_command_timeout())
    if not result.get("success"):
        return json.dumps({
            "success": False,
            "vision_disabled": True,
            "error": (
                f"Dream Workbench text snapshot failed: {result.get('error', 'unknown error')}. "
                "Use browser_navigate/dream_open_url to load the Workbench page, then retry "
                "browser_snapshot or the relevant semantic browser tool. Do not fall back to "
                "OS Chrome or screenshot capture."
            ),
        }, ensure_ascii=False)
    ...
    return json.dumps({
        "success": True,
        "vision_disabled": True,
        "mode": "dream-workbench-text-dom",
        "url": data.get("url", ""),
        "title": data.get("title", ""),
        "question": question,
        "analysis": (
            "Screenshot/vision capture was skipped because Dream Desktop is running "
            "the Workbench browser in text/DOM mode. Use the snapshot below as the "
            "source of truth for navigation, clicking, forms, chess/game state, and "
            "page interaction. If a Chess boards section is present, use "
            "browser_chess_state() to read the board and browser_chess_move(from_square, to_square) "
            "to execute the move you choose."
        ),
        "snapshot": snapshot_text,
        "annotate_requested": bool(annotate),
...
def browser_click(ref: str, task_id: Optional[str] = None, label: Optional[str] = None) -> str:
    """
    Click on an element.
    ...
    Args:
        ...
        task_id: Task identifier for session isolation
        label: Optional exact visible label for stale-ref protection
    ...
    # Native agent-browser expects refs. Dream Workbench fallback also accepts
    # visible labels, so only coerce numeric shorthand into @e refs there.
    if _should_use_dream_browser_fallback():
        ref = str(ref or "").strip()
        if re.fullmatch(r"(?:e)?\d+", ref):
            ref = f"@{ref if ref.startswith('e') else 'e' + ref}"
    elif not ref.startswith("@"):
        ref = f"@{ref}"
    
    result = _run_browser_command(effective_task_id, "click", [ref, str(label or "")])
...
def browser_chess_state(task_id: Optional[str] = None) -> str:
    """
    Read the active Workbench chess board as structured text/DOM state.

    This is intentionally neutral: it does not choose a move. The model reads
    the board state, decides a move, then calls browser_chess_move.
    """
    effective_task_id = task_id or "default"
    result = _run_dream_browser_command(effective_task_id, "chess_state", [])

    if result.get("success"):
        return json.dumps({
            "success": True,
            "state": result.get("data", {}),
            "instructions": "Choose a move yourself from this board state, then call browser_chess_move(from_square, to_square). If it is not your turn and the game is active, call browser_chess_wait_turn() and continue when it returns."
        }, ensure_ascii=False)
    return json.dumps({
        "success": False,
        "error": result.get("error", "Failed to read chess board state")
    }, ensure_ascii=False)


def browser_chess_move(
    from_square: str,
    to_square: str,
    ...
    """
    Move a chess piece on the active Workbench board by algebraic squares.

    This is a semantic DOM-board operation for text-only models. It computes
    square centers from the live board geometry and sends clicks inside the
    Workbench preview without relying on screenshots.
    """
    effective_task_id = task_id or "default"
    from_square = str(from_square or "").strip().lower()
    to_square = str(to_square or "").strip().lower()
    promotion_value = str(promotion or "").strip().lower()

    result = _run_dream_browser_command(
        effective_task_id,
        "chess_move",
        [from_square, to_square, promotion_value],
    )
    ...
    if result.get("success"):
        return json.dumps({
            ...
            "state": result.get("data", {}),
            "instructions": "If the game is still active and it is not your turn, call browser_chess_wait_turn(), then choose and execute the next legal move. Do not stop after one move."
        }, ensure_ascii=False)
    state_probe = _run_dream_browser_command(effective_task_id, "chess_state", [], timeout=5)
    return json.dumps({
        "success": False,
        "error": result.get("error", f"Failed to move chess piece {from_square}-{to_square}"),
        "state": state_probe.get("data", {}) if state_probe.get("success") else {},
        "instructions": (
            "The attempted move was not applied. Read the returned state, move only a piece "
            "owned by controlledColor/playAs, and retry with browser_chess_move. Do not use "
            "browser_press or repeat the opponent's last move."
        )
    }, ensure_ascii=False)


def browser_chess_wait_turn(timeout_seconds: Optional[float] = None, task_id: Optional[str] = None) -> str:
    """
    Wait until the active chess board reports that the controlled side can move.

    This keeps a real game flowing after a successful move instead of ending the
    task with a passive "waiting for opponent" summary.
    """
    effective_task_id = task_id or "default"
    timeout_value = 30.0
    try:
        if timeout_seconds is not None:
            timeout_value = max(0.8, min(float(timeout_seconds), 120.0))
    except (TypeError, ValueError):
        timeout_value = 30.0
    ...
    if result.get("success"):
        return json.dumps({
            "success": True,
            "state": result.get("data", {}),
            "instructions": "It is your turn now. Choose a legal move from the returned board state and call browser_chess_move(from_square, to_square). Continue this wait/move loop until the game ends or the user stops."
        }, ensure_ascii=False)
    return json.dumps({
        "success": False,
        "error": result.get("error", "Timed out waiting for your chess turn")
    }, ensure_ascii=False)


def browser_click_square(square: str, task_id: Optional[str] = None) -> str:
    """
    Click a chess square on the active Workbench board.
    Prefer browser_chess_move for full moves.
    """
    effective_task_id = task_id or "default"
    square = str(square or "").strip().lower()
    result = _run_dream_browser_command(effective_task_id, "click_square", [square])
    ...
    return json.dumps({
        "success": False,
        "error": result.get("error", f"Failed to click chess square {square}")
    }, ensure_ascii=False)
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    if _should_use_dream_browser_fallback():
        return _dream_browser_text_snapshot_for_vision(question, annotate, effective_task_id)
...
def check_browser_requirements() -> bool:
    ...
    *and* the provider's required credentials must be present.
    ...
    # Embedded Dream Desktop can provide a Playwright-backed browser fallback.
    # This is intentionally checked before agent-browser so the bundled desktop
    # app can advertise browser capability without requiring a separate CLI.
    if _should_use_dream_browser_fallback():
        return True
...
registry.register(
    ...
    handler=lambda args, **kw: browser_click(ref=args.get("ref", ""), task_id=kw.get("task_id"), label=args.get("label", "")),
    check_fn=check_browser_requirements,
    emoji="👆",
)
registry.register(
    name="browser_chess_state",
    toolset="browser",
    schema=_BROWSER_SCHEMA_MAP["browser_chess_state"],
    handler=lambda args, **kw: browser_chess_state(task_id=kw.get("task_id")),
    check_fn=check_browser_requirements,
    emoji="♟️",
)
registry.register(
    name="browser_chess_move",
    toolset="browser",
    schema=_BROWSER_SCHEMA_MAP["browser_chess_move"],
    handler=lambda args, **kw: browser_chess_move(
        from_square=args.get("from_square", ""),
        to_square=args.get("to_square", ""),
...
registry.register(
    name="browser_chess_wait_turn",
    toolset="browser",
    schema=_BROWSER_SCHEMA_MAP["browser_chess_wait_turn"],
    handler=lambda args, **kw: browser_chess_wait_turn(
...
registry.register(
    name="browser_click_square",
    toolset="browser",
    schema=_BROWSER_SCHEMA_MAP["browser_click_square"],
    handler=lambda args, **kw: browser_click_square(square=args.get("square", ""), task_id=kw.get("task_id")),
    check_fn=check_browser_requirements,
    emoji="♟️",
)
registry.register(

...
_PATH_GLYPH_TRANSLATION = str.maketrans({
    "\uf03a": ":",
    "\uff1a": ":",
    "\ufe55": ":",
    "\ua789": ":",
...
def normalize_dream_path_text(path_value: str) -> str:
    """Normalize model-emitted path glyphs before shell file operations."""
    value = str(path_value or "").strip().translate(_PATH_GLYPH_TRANSLATION)
    value = re.sub(r"^([a-zA-Z])\s*:\s*[\\/]+", r"\1:/", value)
    if os.name == "nt":
        wsl_match = re.match(r"^[/\\]mnt[/\\]([a-zA-Z])(?:[/\\](.*))?$", value, re.IGNORECASE)
        msys_match = re.match(r"^[/\\]([a-zA-Z])(?:[/\\](.*))?$", value)
        drive_match = wsl_match or msys_match
        if drive_match:
...
class ShellFileOperations(FileOperations):
    ...
    def _expand_path(self, path: str) -> str:
        ...
        """
        path = normalize_dream_path_text(path)
        if not path:
            return path
        ...
        if path.startswith('~'):
            ...
            if result.exit_code == 0 and result.stdout.strip():
                ...
                if path == '~':
                    ...
                    # Replace ~ with home. Normalize Windows-style separators
                    # because Dream Desktop can pass paths from a Windows UI
                    # even when Hermes is executing through a POSIX shell.

...
# Minimum context length accepted by this Dream Server build when running the
# vendored Hermes Agent. Upstream Hermes defaults to 64K because large tool
# schemas and long coding sessions need that much working memory. Dream Server
# can run smaller local models for testing; this lowers only the admission gate,
# not the model's reported context window.
MINIMUM_CONTEXT_LENGTH = int(os.getenv("HERMES_MINIMUM_CONTEXT_LENGTH", "16384"))

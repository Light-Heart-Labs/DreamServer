...
| `simulate-installers.sh` | [test-linux.yml](../.github/workflows/test-linux.yml), `Makefile` | Runs dry-run simulations of Linux, macOS, and Windows installers, generates JSON/Markdown summary artifacts under `artifacts/installer-sim/`. |
...
# Individual checks
bash scripts/check-compatibility.sh    # from repo root
bash scripts/check-release-claims.sh   # from repo root
```
